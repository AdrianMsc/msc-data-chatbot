...
const LoginPage = () => {
	...
	return (
		<>
				...
				<div className="max-w-md min-w-96 mx-auto pt-10 pb-6 px-6 shadow-md rounded bg-white items-center flex flex-col justify-center ">
					...
					<form onSubmit={handleSubmit} className="space-y-4 w-full">
						<label className="flex flex-col gap-3 relative">
							<FontAwesomeIcon icon={faUser} className=" text-monochromes-grey_xlight absolute top-4 left-3" />
							<input
								type="email"
								...
								required
							/>
							<span className="self-end text-sm text-monochromes-grey_dark">Forgot username?</span>
						</label>

						<label className="flex flex-col gap-3 relative">
							<FontAwesomeIcon icon={faLock} className=" text-monochromes-grey_xlight absolute top-4 left-3" />
							<input
								type="password"
								...
								required
							/>
							<span className="self-end text-sm text-monochromes-grey_dark">Forgot password?</span>
						</label>
						...
						<button
							...
							className={`w-full py-2 rounded-full transition text-white ${
								loading || isAuthenticated ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'
							}`}
						>
							{isAuthenticated ? 'Already logged in' : loading ? 'Loading...' : 'Login'}
						</button>
			...
			<footer className="p-4 bg-white  text-center text-sm">© 2000 - 2023 MSC Industrial Direct Co., Inc.</footer>
		</>
	);
};
